...
$query =mysqli_query($con,"select * FROM questions where id=$i");
if(!$query)
{
  echo"not working";
}
...
if(isset($_POST['submit'])) // when click on Update button
{
  ...
  $sem=$_POST['semester'];
  $sub=$_POST['subject'];
    $edit = mysqli_query($con,"update questions set file='$fileName',description='$description',class='$class',semester='$sem',subject='$sub' where id='$i'");
	
    if($edit)
    {
       // mysqli_close($con); // Close connection
        header("location:all_questions.php"); // redirects to all records page
